...
let getTripBtn = document.querySelector('#getTripData');
getTripBtn.addEventListener('click',()=>{
  let tripBox = document.querySelector('.tripBox');
  // let tfrom = tripBox.querySelector('select[name="from"]').value;
  // let tto = tripBox.querySelector('select[name="to"]').value;
  // let tdate = tripBox.querySelector('input[name="date"]').value;
  let tfrom = 'Port Blair';
  let tto = 'Swaraj Dweep';
  let tdate = '21-12-2023';
  let array = [tfrom,tto,tdate];

  window.parent.postMessage({ action: 'callFunctionInVelo', funName: 'getTripData', array:array }, '*');
});
